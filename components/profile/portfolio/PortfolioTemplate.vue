  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid && this.activeCheckedList.length > 0) {
    ...
    async handleRemove(e, name) {
      if (this.fileList[name][0]?.id) {
        const res = await this.__DELETE_FILE({
          id: this.fileList[name][0]?.id,
          type: "work",
        });
        if (res) this.fileList[name] = [];
      } else {
        this.fileList[name] = [];
      }
    },
